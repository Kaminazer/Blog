                ...
                <div class="card col-9">
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover text-nowrap ">
                            ...
                            @if($comments->isEmpty())
                                <tr>
                                    <td colspan="3" class="text-center">{{__("No comments")}}</td>
                                </tr>
                            @else
                                @foreach($comments as $comment)
                                    <tr>
                                        <td>{{$comment->id}}</td>
                                        <td>
                                            <a href="{{route('post.show', $comment->id)}}">{{$comment->message}}</a>
                                        </td>
                                        <td>
                                            <a class="text-success" href="{{route('profile.comment.edit', $comment->id)}}">
                                        ...
                                        <td>
                                            <form action="{{route('profile.comment.destroy', $comment->id)}}" method="post">
                                                @csrf
                                                @method('delete')
